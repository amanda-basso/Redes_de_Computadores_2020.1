...
for j in range(8):
    ...
    tests = [
        ...
        ('204.54.91.%d'%(192|(a>>3)), gw10),
        ('204.54.91.%d'%(64|(a>>3)), gw10),
        ('204.54.91.%d'%(128|(a>>3)), gw10),
        ('1.2.3.4', gw9),
        ('5.6.7.8', gw10),
        ('9.10.11.12', gw10),
    ]
...
# Testa máscaras variadas sem rota padrão definida
for j in range(64):
    gw1, gw2, gw3, gw4, gw5 = \
        tuple(rand_ip() for i in range(5))
    tbl = [
        ('200.0.0.0/8', gw1),
        ('200.1.0.0/16', gw2),
        ('200.1.128.0/17', gw3),
    ...
    random.shuffle(tbl)
    rede.definir_tabela_encaminhamento(tbl)
    a,b,c,d = tuple(random.randint(0, 255) for i in range(4))
    tests = [
        ('200.0.%d.%d'%(b,c), gw1),
        ('200.%d.%d.%d'%(min(a+2,255),b,c), gw1),
        ('200.1.%d.%d'%(b>>1,c), gw2),
        ('200.1.128.%d'%c, gw3),
        ('200.1.129.%d'%c, gw4),
        ('200.1.130.%d'%c, gw5),
        ('200.1.%d.%d'%(max(131,b),c), gw3),
        ('%d.%d.%d.%d'%(min(a,199),b,c,d), None),
        ('%d.%d.%d.%d'%(max(a,201),b,c,d), None),
    ]
    random.shuffle(tests)
    for dest, gw in tests:
        got_gw = enviar_datagrama(dest)
