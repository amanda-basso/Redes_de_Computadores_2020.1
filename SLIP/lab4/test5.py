...
def caso(entrada, saida=[], erro=False):
    if erro:
        enlace.registrar_recebedor(recebedor_erro)
    else:
...
caso([b'\xc0\xdb\xdd\xc0'], erro=True)
caso([b'\xdb\xdd\xc0'], [b'\xdb'])
caso([b'\xdb\xdc\xc0'], erro=True)
caso([b'\xc0\xdb\xdc\xc0'], erro=True)
caso([b'\xdb\xdc\xc0'], [b'\xc0'])

caso([b'A\xdb\xdd\xc0'], erro=True)
caso([b'\xc0\xdb\xddB\xc0'], erro=True)
caso([b'CD\xdb\xdd\xc0'], [b'CD\xdb'])
caso([b'EF\xdb\xdcGHI\xc0'], [b'EF\xc0GHI'])
caso([b'\xc0JKL\xdb\xdc\xc0'], [b'JKL\xc0'])
caso([b'\xdb\xdcMNOP\xc0'], erro=True)

# Casos de teste com um único quadro quebrado em pedaços
caso([b'\xc0\xdb', b'\xdd\xc0'], [b'\xdb'])
caso([b'\xc0', b'\xdb', b'\xdd', b'\xc0'], erro=True)
...
caso([b'\xc0G\xdb', b'\xdcHI\xc0'], erro=True)
...
caso([b'\xc0ABC\xc0\xc0DE\xc0'], erro=True)
...
caso([b'\xc0\xdb\xdc',b'ABC',b'\xc0',b'\xc0',b'D',b'E',b'\xc0'], erro=True)
